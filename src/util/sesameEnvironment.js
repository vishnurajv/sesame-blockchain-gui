...
const getSesameVersion = () => {
  let version = null;
  const exePath = getExecutablePath('sesame');
  // first see if we can get a sesame exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or sesame is on the path
    try {
      version = child_process
