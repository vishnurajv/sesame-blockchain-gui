...
global.daemon_rpc_ws = `wss://${self_hostname}:44400`;
...
function loadConfig(version) {
  try {
    ...
    // 1) SESAME_ROOT environment variable
    // 2) version passed in and determined by the `sesame version` call

    // check if SESAME_ROOT is set. it overrides everything else
    const config_root_dir =
      'SESAME_ROOT' in process.env
        ? process.env.SESAME_ROOT
        : path.join(os.homedir(), '.sesame', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 44400); // jshint ignore:line
