...
import sesameEnvironment from '../util/sesameEnvironment';
import sesameConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!sesameEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    sesameConfig.loadConfig(sesameEnvironment.getSesameVersion());
    ...
    const createWindow = () => {
      ...
      if (!sesameConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !sesameConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (sesameConfig.manageDaemonLifetime()) {
        sesameEnvironment.startSesameDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Sesame Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Sesame-Network/sesame-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sesame-Network/sesame-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sesame-Network/sesame-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sesame-Network/sesame-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sesame-Network/sesame-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/sesame_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/sesame_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Sesame Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Sesame' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Sesame Blockchain' }),
            click: () => {
              openAbout();
            },
          },
